class CakeStage extends HTMLElement {
  connectedCallback() {
    this.innerHTML = `
      ...
      <div id="cake-section"
        ...
        <div class="relative w-48 h-52 sm:w-56 sm:h-60 mx-auto bg-gradient-to-b from-pink-50 to-pink-100/50 border-4 border-pink-200 rounded-3xl flex flex-col items-center justify-end p-4 sm:p-5 shadow-inner overflow-hidden">
          ...
          <!-- Animasi Asap (Hidden secara bawaan) -->
          ...
          <!-- Layer Bawah Kue (Personalized Title) -->
        ...
        <div id="wish-message"
          ...
          🎉 Horeee! Lilinnya mati! Semoga semua doa & harapanmu dikabulkan ya sayang! ❤️
        </div>
    ...
    // Bind Event Listener Internal (Lebih Rapi & Safe)
    this.querySelector("#blow-btn").addEventListener("click", () => this.blowCandle());
  }
  ...
  blowCandle() {
    ...
    // Dispatch custom event (Opsi jika ingin unlock Stage 2 secara otomatis)
    this.dispatchEvent(new CustomEvent("wish-granted", { bubbles: true }));
  }
}
